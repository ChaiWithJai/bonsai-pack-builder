...
demo proved a narrow workflow: capture a synthetic note, scrub identifiers inside
a first-party edge boundary, run a verifier gate, and send only a clean structured
record to Slack. This repo generalizes the lesson into a pack-authoring harness:

```text
workflow interview -> draft pack -> synthetic evals -> gates -> evidence -> contribution
```
